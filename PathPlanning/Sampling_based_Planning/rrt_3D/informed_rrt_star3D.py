...
from utils3D import getDist, sampleFree, steer, isCollide, isinside, near, nearest, path
...
class IRRT:
    ...
    def __init__(self, show_ellipse=False):
        ...
        self.Parent = {}
        self.Path = []
        self.N = 16000  # used for determining how many batches needed
        self.ind = 0
        self.i = 0
    ...
    def Informed_rrt(self, Timer):
        ...
        c = 1
        while time.time() - Timer.st_time < 102:
            if self.ind % 100 == 0:
                print("cur iter is " + str(self.ind))
                self.convergence_curve.append([time.time() - Timer.st_time, self.getCBest()])
                self.success_curve.append([time.time() - Timer.st_time, (self.getCBest() <= 1000)])
...
def main(seed):
    ...
    A.Informed_rrt(IRRT_timer)
    IRRT_timer.show()
    print("the cost curve is ")
    print(A.success_curve)
